...
# Streamlit has the great system to cache the data, So that it caches the data so that it does not loads data every time we do an refresh
...
def load_data():
    ...
    df_agg = pd.read_csv(
        './archive/Aggregated_Metrics_By_Video.csv').iloc[1:, :]
    ...
    df_agg_sub = pd.read_csv(
        './archive/Aggregated_Metrics_By_Country_And_Subscriber_Status.csv')
...
# "Pivot table" is used to store the values in the ordered form
...
######################################################################
# Starting building Streamlit App
######################################################################
...
if add_sidebar == 'Aggregate Metrics':
    ...
    st.write("Ken Jee Yoytube Aggregated Data")
    ...
    metric_median6mo = df_agg_metrics[df_agg_metrics['Video publish time']
                                      >= metric_date_6mo].median()

    metric_median12mo = df_agg_metrics[df_agg_metrics['Video publish time']
                                       >= metric_date_12mo].median()
    ...
    for i in metric_median6mo.index:
        with columns[count]:
            delta = (metric_median6mo[i] -
                     metric_median12mo[i]/metric_median12mo[i])
            st.metric(label=i, value=round(
                metric_median6mo[i], 1), delta="{:.2%}".format(delta))
            count += 1
            if count >= 5:
                count = 0
...
if add_sidebar == 'Individual Video Analysis':
    ...
    fig2.update_layout(title='View comparision first 30 days',
